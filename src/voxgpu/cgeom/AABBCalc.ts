...
class AABBCalc {
	...
	IntersectRL(ltv: IVector3, lpv: IVector3, ab: IAABB): boolean {
		let f: number = 0;
	...
	static IntersectionRL1(ltv: IVector3, lpv: IVector3, ab: IAABB, outV: IVector3): boolean {
		let f: number = 0;
		let tmin: number = (ab.min.x - lpv.x) / ltv.x;
		let tmax: number = (ab.max.x - lpv.x) / ltv.x;
		//console.log("AABB::IntersectRL uses...");
	...
	//
	static IntersectionRL2(ltv: IVector3, lpv: IVector3, ab: IAABB, outV: IVector3): boolean {
		// 计算包围球
