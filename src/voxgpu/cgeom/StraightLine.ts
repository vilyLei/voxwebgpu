...
export class StraightLine extends AbsGeomBase {
	...
	//@param			spCV		空间中的一点
	//
	static CalcPVSquaredDis2(ltv: Vector3, lpv: Vector3, spCV: Vector3): number {
		let pv: Vector3 = StraightLine.__tAv;
		pv.x = spCV.x - lpv.x;
		pv.y = spCV.y - lpv.y;
		pv.z = spCV.z - lpv.z;
		pv.w = pv.dot(ltv);
	...
	//计算空间中一点到空间某直线的距离
	static CalcPVDis(ltv: Vector3, lpv: Vector3, spCV: Vector3): number {
		let pv: Vector3 = StraightLine.__tAv;
		pv.x = spCV.x - lpv.x;
		pv.y = spCV.y - lpv.y;
		pv.z = spCV.z - lpv.z;
	 ...
	 * @param			spCV			空间中的一点
	 * */
	static CalcPVCloseV2(lpv: Vector3, ltv: Vector3, spCV: Vector3, outV: Vector3): void {
		outV.x = spCV.x - lpv.x;
		outV.y = spCV.y - lpv.y;
		outV.z = spCV.z - lpv.z;
	 ...
	 */
	static IntersectionCopSLV2(lapv: Vector3, latv: Vector3, lbpv: Vector3, lbtv: Vector3, outV: Vector3): void {
		Vector3.Cross(latv, lbtv, AbsGeomBase.__tV1);
		Vector3.Cross(latv, AbsGeomBase.__tV1, AbsGeomBase.__tV2);
	 ...
	 */
	static CalcTwoSLCloseV2(lapv: Vector3, latv: Vector3, lbpv: Vector3, lbtv: Vector3, outV: Vector3): void {
		Vector3.Cross(latv, lbtv, AbsGeomBase.__tV1);
		Vector3.Cross(latv, AbsGeomBase.__tV1, AbsGeomBase.__tV2);
		AbsGeomBase.__tV2.normalize();
		PlaneCalc.IntersectionSLV2(AbsGeomBase.__tV2, AbsGeomBase.__tV2.dot(lapv), lbpv, lbtv, outV);
	...
	static CalcTwoSLDualCloseV2(lapv: Vector3, latv: Vector3, lbpv: Vector3, lbtv: Vector3, outVa: Vector3, outVb: Vector3): void {
		Vector3.Cross(latv, lbtv, AbsGeomBase.__tV1);
		Vector3.Cross(latv, AbsGeomBase.__tV1, AbsGeomBase.__tV2);
		AbsGeomBase.__tV2.normalize();
