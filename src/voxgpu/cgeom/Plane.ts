...
class Plane extends AbsGeomBase {
	...
	nv = new Vector3(0.0, 1.0, 0.0);
	distance = 0.0;
	intersectBoo = false;
	intersectStraightLinePos(straightL: StraightLine, outV: Vector3): number {
	...
	intersectStraightLinePos2(sl_pos: Vector3, sl_tv: Vector3, outV: Vector3): number {
		// intersection or parallel
	...
	intersectRadialLinePos(radL: RadialLine, outV: Vector3): number {
	...
	intersectRadialLinePos2(rl_pos: Vector3, rl_tv: Vector3, outV: Vector3): number {
	...
	containsPoint(pos: Vector3): number {
	...
	intersectSphere(cv: Vector3, radius: number): number {
	...
	intersectAABB(minV: Vector3, maxV: Vector3): number {

		this.intersectBoo = false;

		let pv: Vector3 = AbsGeomBase.__tV0;
	...
	static PlaneIntersectSphere(pnv: Vector3, pdis: number, cv: Vector3, radius: number): void {
