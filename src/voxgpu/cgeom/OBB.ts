...
class OBB implements IOBB {
	 ...
	 */
	readonly axes = [new Vector3(), new Vector3(), new Vector3()];
	readonly extents = new Float32Array(3);
	/**
	 * half length of these three axes
	 */
	readonly extent = new Vector3();
	...
	getClosePosition(pv: Vector3, result: Vector3 = null): Vector3 {
		const et = this.extent;
	...
	intersectPlane(planeNV: Vector3, planeDis: number): boolean {
		const et = this.extent;
		const axes = this.axes;
