...
class CurveSrcCalc {
	 ...
	 * @param v2 终点
	 */
	static CalcBezier2(vs: number[], tot: number, v0: Vector3, ctrlV: Vector3, v1: Vector3): void {
	 ...
	 * 计算二次Bezier曲线
	 */
	static CalcBezier2ByProgress(vs: number[], k: number, v0: Vector3, v1: Vector3, v2: Vector3): void {
		//b(t) = (1-t)*(1-t)*p0 + 2*t*(1-t)*p1 + t * t * p2;
	 ...
	 * @param v3 终点
	 */
	static CalcBezier3(vs: number[], tot: number, v0: Vector3, v1: Vector3, v2: Vector3, v3: Vector3): void {
	 ...
	 * 计算三次Bezier曲线
	 */
	static CalcBezier3ByProgress(vs: number[], k: number, v0: Vector3, v1: Vector3, v2: Vector3, v3: Vector3): void {
		//B(t) = p0*(1-t)^3 + 3*p1*t*(1-t)^2+3*p2*t^2*(1-t) + p3 * t^3
	 ...
	 */
	static CalcBezier3ByTV(vs: number[], tot: number, v0: Vector3, tv0: Vector3, v3: Vector3, tv3: Vector3): void {
		let i: number = 0;
		let px: number = 0;
		let py: number = 0;
		let pz: number = 0;
...
class CurveBase {
	 ...
	 * 曲线的起点
	 */
	begin: Vector3 = new Vector3();
	/**
	 * 曲线的终点
	 */
	end: Vector3 = new Vector3(100, 0, 0);
	constructor() { }
...
class Bezier2Curve extends CurveBase {
	 ...
	 * 计算二次Bezier曲线的y值
	 * */
	calcBezier2Y(vs: number[], tot: number, v0: Vector3, v1: Vector3, v2: Vector3): void {
		//b(t) = (1-t)*(1-t)*p0 + 2*t*(1-t)*p1 + t * t * p2;
	...
	private m_direcNV: Vector3 = new Vector3();
	private m_direcTV: Vector3 = new Vector3();
	/**
	...
	calcCurveChangeYData(total: number, y0: number, y1: number, factor: number, amplitude: number, vs: number[] = null): number[] {
		...
		let dis: number = Vector3.Distance(this.begin, this.end);
...
class Bezier3Curve extends CurveBase {
	 ...
	 * 控制点A
	 */
	ctrAPos: Vector3 = new Vector3(40, 40, 0);
	/**
	 * 控制点B
	 */
	ctrBPos: Vector3 = new Vector3(70, 70, 0);
	/**
	 * 重新计算
	 */
	updateCalc(): void {
