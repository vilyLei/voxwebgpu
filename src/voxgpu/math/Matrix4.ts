...
class Matrix4 implements IMatrix4 {
	...
	appendRotationPivot(radian: number, axis: Vector3, pivotPoint: Vector3 = null): void {
		if (pivotPoint == null) {
	...
	decompose(orientationStyle: number = OrientationType.EULER_ANGLES): Vector3[] {
		...
		mr.copyFrom(this);
		///*
		let pos = new Vector3(rfs[12], rfs[13], rfs[14]);
	...
	pointAt(pos: Vector3, at: Vector3, up: Vector3): void {

		//TODO: need optimize
		if (at == null) at = new Vector3(0.0, 0.0, -1.0);
	...
	deltaTransformVector(v3: Vector3): Vector3 {
		let x: number = v3.x;
		let y: number = v3.y;
		let z: number = v3.z;
		return new Vector3(
	...
	transformOutVector(v3: Vector3, out_v3: Vector3): void {
		let x: number = v3.x;
		let y: number = v3.y;
		let z: number = v3.z;
		let fs = this.mLocalFS32;
	...
	transformVector3Self(v3: Vector3): void {
		let x: number = v3.x;
		let y: number = v3.y;
		let z: number = v3.z;
		let fs: Float32Array = this.mLocalFS32;
	...
	transformVectorSelf(v3: Vector3): void {
		let x: number = v3.x;
		let y: number = v3.y;
		let z: number = v3.z;
		let fs: Float32Array = this.mLocalFS32;
	...
	lookAtRH(eyePos: Vector3, atPos: Vector3, up: Vector3): void {
		this.identity();
		let f: Vector3 = atPos.subtract(eyePos);
		f.normalize();
		let s: Vector3 = f.crossProduct(up);
		s.normalize();
		let u: Vector3 = s.crossProduct(f);
		s.w = -s.dot(eyePos);
		u.w = -u.dot(eyePos);
		f.w = f.dot(eyePos);
		f.negate();
	...
	lookAtLH(eyePos: Vector3, atPos: Vector3, up: Vector3): void {
		this.identity();
		let f: Vector3 = atPos.subtract(eyePos);
		f.normalize();
		let s: Vector3 = f.crossProduct(up);
